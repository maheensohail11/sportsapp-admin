...
function CricScorer (props) {
  var history= useHistory();
    ...
    const handleWide= async e =>{
        current_over.push("1wd");
        await setOv(current_over).then(console.log("in SetOv"));
    ...
    const handleNoball= async e =>{
      current_over.push("1nb");
      await setOv(current_over).then(console.log("in SetOv"));
      var runs = parseInt(e.target.value, 10);
      runs_per_over += runs;
      total_runs_1st_innings += runs;
  ...
  <h2>Select Bowler: </h2>
    ...
    <br />
        ...
        <div>
          ...
          <button onClick={e => handleWide(e, "value")} value="1"> Wide </button>
          <button onClick={e => handleNoball(e, "value")} value="1"> No Ball </button>
          ...
          <h6> Current Over </h6>
          <div className='divbord' style={{ width:500, marginLeft:440, height: 48, marginTop:15 }}>
          <div style={{justifyContent: 'center', marginLeft: 0}}>
                <Table bordered style={{width: 20}} >
